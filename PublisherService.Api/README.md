...
> A modern, high-performance read-only GraphQL API microservice for Publisher data.

This project is part of the Bookswagon Core Architecture, designed to deliver fast, scalable, and efficient publisher data using state-of-the-art .NET technologies.
...
* **Domain Layer**: Plain C# entities (e.g., `Publisher`) mapping to database tables.
* **Application Layer**: Centralizes business rules and interface definitions (e.g., `IPublisherRepository`, `IPublisherService`).
...
* **`IQueryable` Returns**: Repositories safely return `IQueryable`, enabling Hot Chocolate to natively append filtering and pagination directly to the SQL query *before* database execution.
...
This API exposes two highly optimized Hot Chocolate endpoints. Here are examples you can try directly in Banana Cake Pop:

### 1. `publisherById`
Fetches a single publisher by their unique database `id` (using standard Service mapping).
```graphql
query {
  publisherById(id: 22) {
    publisherId
...
### 2. `searchPublishers`
Supports full pagination, projection, and dynamic filtering by translating GraphQL directly to SQL queries utilizing EF Core's `IQueryable`.
```graphql
query {
  searchPublishers(name: "penguin", first: 5, after: "", before: "") {
    nodes {
      publisherId
      companyName
      description
